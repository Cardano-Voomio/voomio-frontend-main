...
import { FadeImg } from "../Fades/Fades";
...
const AssetImageFigure = ({ asset, setShow, show_trigger, width, no_figure, className }) => {
  return (
    <>
      {
        no_figure ? (
          <FadeImg className={className} src={get_asset_image_source(asset.image)} alt={asset.name} style={{ width: width, height: width }} />
        ) :
          setShow ? (
            <div onClick={() => setShow(show_trigger ? show_trigger : false)} style={{ cursor: "pointer" }}>
              <LazyLoad className="image is-square">
                <Image src={get_asset_image_source(asset.image)} alt={asset.name} style={{ width: width, height: width }} />
              </LazyLoad>
            </div>
          ) : (
            <LazyLoad className="image is-square">
