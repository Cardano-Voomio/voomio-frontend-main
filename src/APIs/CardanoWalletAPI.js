...
class CardanoWalletAPI {
    ...
    async getBalance() {
        ...
        const minAda = this.S.min_ada_required(countedValue, false, this.S.BigNum.from_str(protocolParameter.minUtxo));
    ...
    async _txBuilder({
        ...
        if (metadata) {
            ...
            const generalMetadata = this.S.GeneralTransactionMetadata.new();
            Object.entries(this.Metadata).map(([MetadataLabel, Metadata]) => {
                generalMetadata.insert(
                    this.S.BigNum.from_str(MetadataLabel),
                    this.S.encode_json_str_to_metadatum(JSON.stringify(Metadata), 0)
                );
            ...
            this.aux.set_metadata(generalMetadata)
