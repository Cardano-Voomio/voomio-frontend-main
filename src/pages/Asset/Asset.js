...
import AssetImageFigure from "Components/AssetImageFigure/AssetImageFigure"

import { fromLovelace } from "utils/converter";

import "./Asset.css";

const Asset = ({ showAsset, chaintype }) => {
  ...
  const [adaAsset, setAdaAsset] = useState(false);
  ...
  useEffect(() => {
    ...
    if (policy_id in state_collection.policies_assets) {
      if (asset_id in state_collection.policies_assets[policy_id]) {
        setAdaAsset(state_collection.policies_assets[policy_id][asset_id]);
        console.log("[dream log] Asset useEffet origin Asset:", state_collection.policies_assets[policy_id][asset_id]);
      } else {
        query_asset = true;
      }
    } else {
    ...
    if (chaintype == "ADA" && query_asset && !state_collection.loading) {
      dispatch(getAsset(asset_id, (res) => { }));
    }

    console.log("[dream log] Asset useEffet Asset:", showAsset);

    setAsset(showAsset);
  }, [policy_id, asset_id, state_collection, showAsset]);
        ...
        <div className="level-left">
          <div className="level-item asset_name_block">
            ...
            {thisCollection ? (
              ...
              // <Link to={`/collection/${asset.id}`}>
              <>
                {asset.id}
              </>
              // </Link>
            )}
            {thisCollection.is_voomio_verified ? (
...
const Listing = ({
  ...
  let is_owner = false;
  if (chaintype == "ADA") {
    if (adaAsset.asset in state_wallet.data.assets) {
      is_owner = true;
    }
    if (adaAsset.status && state_wallet.connected) {
      if (adaAsset.submittedBy === state_wallet.data.address) {
        is_owner = true;
      }
    }
  }
...
const PurchaseAsset = ({
  ...
  const [showTab, setShowTab] = useState(
    asset.status ? (asset.status ? "buy" : "offer") : "offer"
  );
  ...
  function list_this_token(price) {
    if (chaintype == "ADA")
      asset_add_offer(state_wallet, adaAsset, price, (res) => {
  ...
  function purchase_this_token() {
    if (chaintype == "ADA")
      purchase_token(state_wallet, adaAsset, (res) => {
        successful_transaction(res);
      });
    else
        ...
        {showTab === "offer" ? (
          <div className="field has-addons">
            <div className="control has-icons-left is-expanded">
              ...
              />
              {chaintype == "ADA" ? <span className="icon is-medium is-left">&nbsp;₳</span> : <span className="icon is-medium is-left">&nbsp;SOL</span>}
              {!state_wallet.connected ? (
            ...
            <div className="control">
              ...
              >
                ...
                {userInputAmount
                  ? (chaintype == "ADA" ? `Offer for ₳${userInputAmount}` : `Offer for SOL${userInputAmount}`)
                  : "Offer a price"}
              </button>
            </div>
          </div>
      ...
      {showModal ? (
        ...
        >
          {showModal === MARKET_TYPE.PURCHASE ? (
            <span>
              Purchased <b>{asset.name}</b>!
            </span>
          ) : showModal === "offer-success" ? (
            <span>
              You made an offer for <b>{asset.name}</b>!
            </span>
          ) : (
            ""
          )}
...
const OwnerListAsset = ({
  ...
  function list_this_token(price) {
    if (chaintype == "ADA")
      list_token(state_wallet, adaAsset, price, (res) => {
        successful_transaction(res);
      });
    else
  ...
  function update_this_listing(price) {
    if (chaintype == "ADA")
      relist_token(state_wallet, adaAsset, price, (res) => {
        successful_transaction(res);
      });
    else
  ...
  function delist_this_token() {
    if (chaintype == "ADA")
      delist_token(state_wallet, adaAsset, (res) => {
        successful_transaction(res);
      });
    else
  ...
  return (
    <div className="card">
      <header className="card-header">
        <p className="card-header-title">
          List {asset.name} for sale in the Marketplace
        </p>
      </header>
      ...
      <div className="card-content">
        ...
        <div className="field has-addons">
          <div className="control has-icons-left is-expanded">
            ...
            />
            {chaintype == "ADA" ? <span className="icon is-medium is-left">&nbsp;₳</span> : <span className="icon is-medium is-left">&nbsp;SOL</span>}
            {chaintype == "ADA" ? state_wallet.provider.collateral.length === 0 ? (
              <p className="help">
                Fund the wallet and add collateral (option in Nami).
              </p>
            ) : (
          ...
          <div className="control">
            ...
            >
              {userInputAmount ? (chaintype == "ADA" ? `List for ₳${userInputAmount}!` : `List for SOL${userInputAmount}!`) : "List this!"}
      ...
      {showModal ? (
        ...
        >
          {operation === MARKET_TYPE.NEW_LISTING ? (
            <span>
              Listed <b>{asset.name}</b> successfully!
            </span>
          ) : operation === MARKET_TYPE.PRICE_UPDATE ? (
            <span>
              Listing price for <b>{asset.name}</b>{" "}
              updated!
            </span>
          ) : operation === MARKET_TYPE.DELIST ? (
            <span>
              <b>{asset.name}</b> removed from the
              marketplace.
            </span>
          ) : (
            ""
        ...
        <div className="card-content">
          <table className="table">
            <tbody>
              ...
              {/* )} */}
              <tr>
                <th className="attr">Policy ID/Address</th>
                <td>
                    ...
                    {/* <div className="level-right">
                      <div className="level-item">
                        <a
                          ...
                          href={`${urls.cardanoscan}tokenPolicy/${asset.id}`}
                          rel="noreferrer"
                          target="_blank"
                          data-tooltip="Check Cardanoscan"
    ...
    if (asset && show && contentSource == null) {
      ...
      if (asset.files) {
        if (asset.files.length) {
          ...
          if (asset.files[0].mediaType == "text/html") {
            thisContentType = "html";
          }
          else if (asset.files[0].mediaType == "audio/mpeg") {
          ...
          if (thisContentType != "image") {
            ...
            // prepare src link
            if (asset.files[0].src.includes("ipfs://")) {
              let s = ipfs_root + asset.files[0].src.split("ipfs://")[1];
              setContentSource(s);
            } else {
              let s = ipfs_root + asset.files[0].src;
              setContentSource(s);
            }
          }
        }
...
const Transactions = ({ asset, opencnft_get_asset_tx, chaintype }) => {
  const [firstLoad, setFirstLoad] = useState(false);
  const [fetching, setFetching] = useState(false);
  const [transactions, setTransactions] = useState(false);

  function get_txn_voomio(list_transactions) {
    if (asset.events) {
  ...
  function feteh_update() {
    ...
    list_transactions = get_txn_voomio(list_transactions);

    opencnft_get_asset_tx(asset.asset, (res) => {
      setFirstLoad(true);
      if (res.data.items) {
...
function mapStateToProps(state) {
  return {
    showAsset: state.collection.show_asset,
